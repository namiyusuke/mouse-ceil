...
export default class Metaball {
  ...
  initEvents() {
    window.addEventListener('pointermove', (e) => {
      this.mx = e.clientX
      this.my = Config.height - e.clientY
      // マウス位置に一時バブルをスポーン
    ...
    if (sliderCount) {
      sliderCount.addEventListener('input', () => {
        this.activeBubbles = +sliderCount.value
        valCount.textContent = this.activeBubbles
      })
    }
    if (sliderThresh) {
      sliderThresh.addEventListener('input', () => {
        this.mesh.material.uniforms.uThreshold.value = +sliderThresh.value / 10
        valThresh.textContent = this.mesh.material.uniforms.uThreshold.value.toFixed(1)
      })
    }
  }
